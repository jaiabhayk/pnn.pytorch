...
class PerturbResNet(nn.Module):
    def __init__(self, block, nblocks=None, avgpool=None, nfilters=None, nclasses=None, nmasks=None, level=None, filter_size=None,
                        first_filter_size=None, use_act=False, shape=None, group=False, act=None, scale_noise=1, unique_masks=False):
        ...
        layers = [PerturbLayer(in_channels=3, out_channels=nfilters, nmasks=num_masks, level=level*scale_noise,
...
class CifarNet(nn.Module):
    def __init__(self, nfilters=None, nclasses=None, nmasks=None, level=None, filter_size=None, linear=256, group=False,
        ...
        print('\n\nscale_noise:', scale_noise, '\n\n')
        self.first_layers = nn.Sequential(
            PerturbLayer(in_channels=3, out_channels=nfilters, nmasks=nmasks*nfilters, level=level*scale_noise,
                         filter_size=first_filter_size, use_act=use_act, shape=(1, 1, nmasks, 32, 32), group=group, act=self.act),
            PerturbLayer(in_channels=nfilters, out_channels=nfilters, nmasks=nmasks, level=level, filter_size=filter_size,
                         use_act=True, shape=(1, nfilters, nmasks, 32, 32), group=group, act=self.act),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            #nn.MaxPool2d(2, 2, 0),
            #nn.AvgPool2d(2, 2, 0),
            PerturbLayer(in_channels=nfilters, out_channels=nfilters, nmasks=nmasks, level=level, filter_size=filter_size,
                         use_act=True, shape=(1, nfilters, nmasks, 16, 16), group=group, act=self.act),
            PerturbLayer(in_channels=nfilters, out_channels=nfilters, nmasks=nmasks, level=level, filter_size=filter_size,
                         use_act=True, shape=(1, nfilters, nmasks, 16, 16), group=group, act=self.act),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            #nn.MaxPool2d(2, 2, 0),
            #nn.AvgPool2d(2, 2, 0),
            PerturbLayer(in_channels=nfilters, out_channels=nfilters, nmasks=nmasks, level=level, filter_size=filter_size,
                         use_act=True, shape=(1, nfilters, nmasks, 8, 8), group=group, act=self.act),
            PerturbLayer(in_channels=nfilters, out_channels=nfilters, nmasks=nmasks, level=level, filter_size=filter_size,
                         use_act=True, shape=(1, nfilters, nmasks, 8, 8), group=group, act=self.act),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            #nn.MaxPool2d(kernel_size=2, stride=2, padding=1),
...
def resnet18(nfilters, avgpool=4, nclasses=10, nmasks=32, level=0.1, filter_size=0, first_filter_size=0, group=False, scale_noise=1, act='relu', use_act=True, dropout=0.5, unique_masks=False):
        return ResNet(BasicBlock, [2, 2, 2, 2], nfilters=nfilters, avgpool=avgpool, nclasses=nclasses, filter_size=filter_size, first_filter_size=first_filter_size)


def noiseresnet18(nfilters, avgpool=4, nclasses=10, nmasks=32, level=0.1, filter_size=0, first_filter_size=7, group=False, scale_noise=1, act='relu', use_act=True, dropout=0.5, unique_masks=False):
    return NoiseResNet(NoiseBasicBlock, [2, 2, 2, 2], nfilters=nfilters, nchannels=3, pool=avgpool, nclasses=nclasses, level=level, first_filter_size=first_filter_size)


def perturb_resnet18(nfilters, avgpool=4, nclasses=10, nmasks=32, level=0.1, filter_size=0, first_filter_size=0, group=False, scale_noise=1, act='relu', use_act=True, dropout=0.5, unique_masks=False):
    return PerturbResNet(PerturbBasicBlock, [2, 2, 2, 2], nfilters=nfilters, avgpool=avgpool, nclasses=nclasses, group=group, scale_noise=scale_noise,
                        nmasks=nmasks, level=level, filter_size=filter_size, first_filter_size=first_filter_size, act=act, use_act=use_act, unique_masks=unique_masks)


def lenet(nfilters, avgpool=None, nclasses=10, nmasks=32, level=0.1, filter_size=3, first_filter_size=0, group=False, scale_noise=1, act='relu', use_act=True, dropout=0.5, unique_masks=False):
    return LeNet(nfilters=nfilters, nclasses=nclasses, nmasks=nmasks, level=level, filter_size=filter_size, group=group, scale_noise=scale_noise,
                    act=act, first_filter_size=first_filter_size, use_act=use_act, dropout=dropout, unique_masks=unique_masks)


def cifarnet(nfilters, avgpool=None, nclasses=10, nmasks=32, level=0.1, filter_size=3, first_filter_size=0, group=False, scale_noise=1, act='relu', use_act=True, dropout=0.5, unique_masks=False):
    return CifarNet(nfilters=nfilters, nclasses=nclasses, nmasks=nmasks, level=level, filter_size=filter_size, group=group, scale_noise=scale_noise,
                    act=act, use_act=use_act, first_filter_size=first_filter_size, dropout=dropout, unique_masks=unique_masks)
